...
func TestPluginIdentity(t *testing.T) {
	...
	if p.Region() != kernel.RegionInspector {
...
func TestRenderHiddenWhenDisabled(t *testing.T) {
	p, h := New(), newFakeHost()
	p.Init(context.Background(), h)
	h.st.Layout.ShowInspector = false
	// ShowInspector=false → 组件输出空串 → compose 折叠该行。
	if out := p.Render(h, 30); out != "" {
		t.Fatalf("hidden render = %q, want empty", out)
	}
}

func TestRenderVisibleReflectsSessions(t *testing.T) {
	p, h := New(), newFakeHost()
	p.Init(context.Background(), h)
	h.st.Layout.ShowInspector = true
	h.st.Layout.InspectorWidth = 30
	h.st.Gateway.Sessions = []gateway.SessionSummary{{ID: "s1", Title: "demo"}}
	out := p.Render(h, 30)
	if !strings.Contains(out, "demo") {
		t.Fatalf("render should reflect session title, got %q", out)
	}
}

func TestRenderBeforeInitIsSafe(t *testing.T) {
	p := New()
	if out := p.Render(nil, 30); out != "" {
