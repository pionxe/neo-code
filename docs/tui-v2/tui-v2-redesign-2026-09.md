# TUI v2 设计方案 v3.1 — 内核 + 插件（经 5 轮自查 + 1 轮外部审计）

- **状态**: v3.1 审计补丁版 — 已按 issue #12 审计意见修订全部 P0/P1/P2 补丁，待签收入库（TUIv2-00）
...
- **用户已裁决**: ① 真实网关没有的 RPC 一律按 `ErrUnsupported` 显式降级，后端零改动，将来需要时一并补；② 北极星 = 美观 + 低耦合、模块分明，借鉴"万物皆插件"；③ 主题样式通过新增/修改插件自定义；④ 审计裁定（issue #12 评论）：思想批准、十插件保留（真插件 6 + 视图型 4）、confirm 维持内核服务、合入改多 PR、S5 为架构 go/no-go 门。
...
**写权白名单测试（P1 补丁）**：让违规在 CI 可见，而不是靠自觉。三件套：① 槽字段在 state 包内按 owner 分组子结构并标注 owner；② 插件的写路径收敛为 state 包的具名 Mutate 函数（owner 显式入参）；③ grep 脚本检查插件目录中不存在对他人槽字段的直接赋值（出现 `State().他人槽.字段 =` 模式即 CI 失败）+ 一张"插件→槽"所有权表测试锁定注册表。
...
| S1 | 状态指针稳定：Reduce 就地变更，删除 bindComponents（TUIv2-01，已开 issue） | 插件"构造一次终身绑定"的前提 |
...
| S6 | **新增 health 插件（断连重连）—— 架构 go/no-go 门** | **自证"新功能=新插件，内核零改动"。届时仍需改内核 = 架构预言证伪 → 按第 9 节第一行预案回退，不恋战（审计裁定）** |
